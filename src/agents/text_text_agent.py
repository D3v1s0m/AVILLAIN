"""
Agent 1: Claim-Text-Related-Text Evidence Analysis

Retrieves 10 text evidence items based on claim text similarity,
then uses VLM to analyze the evidence and verify textual assertions.
"""
...
from .prompts import AGENT1_PROMPT1, AGENT1_PROMPT2
...
class TextTextAgent(BaseAgent):
    """Agent for analyzing text evidence related to claim text."""
    ...
    def retrieve_text_evidence(
        ...
        if self.use_reranker:
            ...
            candidate_docs = []
            candidate_meta = []  # Store (idx, chunk_id, chunk) for later
    ...
    def _generate_analysis(
        ...
        content = [{"type": "text", "text": AGENT1_PROMPT1.format(speaker, date, claim_text)}]
        ...
        content.append({"type": "text", "text": AGENT1_PROMPT2.format(evidence_text)})
    ...
    def analyze(
        ...
        **kwargs
        ...
        """Analyze claim using text-based evidence retrieval and VLM analysis.

        Agent 1 uses text_related_store (text-text retrieval).
        """
        # Load pre-computed embeddings from text_related store
        embeddings, chunks, pos_to_id = self.load_text_embeddings(claim_id, store_type="text_related")
